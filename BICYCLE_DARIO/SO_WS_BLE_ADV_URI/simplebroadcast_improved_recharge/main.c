...
void sleep_subsec(uint16_t subsec_sleep){
	//IntDisable(INT_EDGE_DETECT);
	//read now
	uint32_t now=AONRTCCurrentCompareValueGet();
	//split now into subsec and sec
	uint32_t nowsubsec=now&0xffff;
	uint32_t nowsec=now>>16;
	//add subsec to now
	if((subsec_sleep+nowsubsec) > 0xffff){
		nowsec++;
		nowsubsec += subsec_sleep-0xffff;
	}else{
		nowsubsec += subsec_sleep;
	}

	uint32_t set=(nowsec<<16) | (nowsubsec&0xffff);
	AONRTCCompareValueSet(AON_RTC_CH0, set);
...
int main(void) {
  ...
  HWREG(AON_EVENT_BASE + AON_EVENT_O_MCUWUSEL) = AON_EVENT_MCUWUSEL_WU0_EV_PAD | AON_EVENT_MCUWUSEL_WU1_EV_RTC_CH0;  //Does not work with AON_EVENT_MCUWUSEL_WU0_EV_PAD4 --> WHY??
...
//Todo: Read sensor values
    ...
    //sleep275ms to wait for sensors ready
    //(2^16)/1000ms*275ms=18022.4
    sleep_subsec(18023);
    int temperature;
    do{
...
//    uint32_t now=AONRTCCurrentCompareValueGet();
//       uint32_t nowsubsec=now&0xff;
//       uint32_t nowsec=now>>16;
//       nowsec++;
//       uint32_t set=(nowsec<<16) | (nowsubsec&0xff);
//       AONRTCCompareValueSet(AON_RTC_CH0, set);
//       AONRTCChannelEnable(AON_RTC_CH0);
